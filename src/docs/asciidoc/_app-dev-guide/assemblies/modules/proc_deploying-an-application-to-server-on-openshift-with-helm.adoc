...
. Create a directory called `charts` in the application _<application_home>_ directory and navigate to it.
+
...
build:
  ...
  contextDir: __<application_home>__                      <3>
deploy:
  replicas: 1                                         <4>
----
...
----
$ helm install __<application_home>__ -f helm.yaml {helmChart}
----
...
----
$ APPLICATION_URL=https://$(oc get route __<application_home>__ --template='{{ .spec.host }}') &&
echo "" &&
